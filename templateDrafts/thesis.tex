...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{include\_graphics}\NormalTok{(}\StringTok{"https://bookdown.org/yihui/rmarkdown{-}cookbook/images/workflow.png"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{https://bookdown.org/yihui/rmarkdown-cookbook/images/workflow.png}
...
\begin{longtable}[]{@{}lr@{}}
\caption{Maximum Delays by Airline}\tabularnewline
\toprule
...
\subsubsection{Other examples of
...
\section{Graphics, References, and Labels}\label{ref-labels}}
...
(\url{https://web.reed.edu/cis/help/latex/index.html})\footnote{@reedweb2007}.
There are three pages on this topic: \emph{bibtex} (which talks about
using BibTeX, at \url{https://web.reed.edu/cis/help/latex/bibtex.html}),
...
\begin{Highlighting}[]
\CommentTok{\# Examples}
\DocumentationTok{\#\# In principle, as not all systems have iconvlist}
\FunctionTok{try}\NormalTok{(utils}\SpecialCharTok{::}\FunctionTok{head}\NormalTok{(}\FunctionTok{iconvlist}\NormalTok{(), }\AttributeTok{n =} \DecValTok{50}\NormalTok{))}

\DocumentationTok{\#\# Not run: }
\DocumentationTok{\#\# convert from Latin{-}2 to UTF{-}8: two of the glibc iconv variants.}
\FunctionTok{iconv}\NormalTok{(x, }\StringTok{"ISO\_8859{-}2"}\NormalTok{, }\StringTok{"UTF{-}8"}\NormalTok{)}
\FunctionTok{iconv}\NormalTok{(x, }\StringTok{"LATIN2"}\NormalTok{, }\StringTok{"UTF{-}8"}\NormalTok{)}

\DocumentationTok{\#\# End(Not run)}

\DocumentationTok{\#\# Both x below are in latin1 and will only display correctly in a}
\DocumentationTok{\#\# locale that can represent and display latin1.}
\NormalTok{x }\OtherTok{\textless{}{-}} \StringTok{"fa}\SpecialCharTok{\textbackslash{}xE7}\StringTok{ile"}
\FunctionTok{Encoding}\NormalTok{(x) }\OtherTok{\textless{}{-}} \StringTok{"latin1"}
\NormalTok{x}
\FunctionTok{charToRaw}\NormalTok{(xx }\OtherTok{\textless{}{-}} \FunctionTok{iconv}\NormalTok{(x, }\StringTok{"latin1"}\NormalTok{, }\StringTok{"UTF{-}8"}\NormalTok{))}
\NormalTok{xx}

\FunctionTok{iconv}\NormalTok{(x, }\StringTok{"latin1"}\NormalTok{, }\StringTok{"ASCII"}\NormalTok{)          }\CommentTok{\#   NA}
\FunctionTok{iconv}\NormalTok{(x, }\StringTok{"latin1"}\NormalTok{, }\StringTok{"ASCII"}\NormalTok{, }\StringTok{"?"}\NormalTok{)     }\CommentTok{\# "fa?ile"}
\FunctionTok{iconv}\NormalTok{(x, }\StringTok{"latin1"}\NormalTok{, }\StringTok{"ASCII"}\NormalTok{, }\StringTok{""}\NormalTok{)      }\CommentTok{\# "faile"}
\FunctionTok{iconv}\NormalTok{(x, }\StringTok{"latin1"}\NormalTok{, }\StringTok{"ASCII"}\NormalTok{, }\StringTok{"byte"}\NormalTok{)  }\CommentTok{\# "fa\textless{}e7\textgreater{}ile"}
\FunctionTok{iconv}\NormalTok{(xx, }\StringTok{"UTF{-}8"}\NormalTok{, }\StringTok{"ASCII"}\NormalTok{, }\StringTok{"Unicode"}\NormalTok{) }\CommentTok{\# "fa\textless{}U+00E7\textgreater{}ile"}

\DocumentationTok{\#\# Extracts from old R help files (they are nowadays in UTF{-}8)}
\NormalTok{x }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"Ekstr}\SpecialCharTok{\textbackslash{}xf8}\StringTok{m"}\NormalTok{, }\StringTok{"J}\SpecialCharTok{\textbackslash{}xf6}\StringTok{reskog"}\NormalTok{, }\StringTok{"bi}\SpecialCharTok{\textbackslash{}xdfc}\StringTok{hen Z}\SpecialCharTok{\textbackslash{}xfc}\StringTok{rcher"}\NormalTok{)}
\FunctionTok{Encoding}\NormalTok{(x) }\OtherTok{\textless{}{-}} \StringTok{"latin1"}
\NormalTok{x}
\FunctionTok{try}\NormalTok{(}\FunctionTok{iconv}\NormalTok{(x, }\StringTok{"latin1"}\NormalTok{, }\StringTok{"ASCII//TRANSLIT"}\NormalTok{))  }\CommentTok{\# platform{-}dependent}
\FunctionTok{iconv}\NormalTok{(x, }\StringTok{"latin1"}\NormalTok{, }\StringTok{"ASCII"}\NormalTok{, }\AttributeTok{sub =} \StringTok{"byte"}\NormalTok{)}
\DocumentationTok{\#\# and for Windows\textquotesingle{} \textquotesingle{}Unicode\textquotesingle{}}
\FunctionTok{str}\NormalTok{(xx }\OtherTok{\textless{}{-}} \FunctionTok{iconv}\NormalTok{(x, }\StringTok{"latin1"}\NormalTok{, }\StringTok{"UTF{-}16LE"}\NormalTok{, }\AttributeTok{toRaw =} \ConstantTok{TRUE}\NormalTok{))}
\FunctionTok{iconv}\NormalTok{(xx, }\StringTok{"UTF{-}16LE"}\NormalTok{, }\StringTok{"UTF{-}8"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
